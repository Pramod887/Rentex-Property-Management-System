...
$query = "SELECT f_id, f_image FROM addflat ORDER BY RAND() LIMIT 1";
...
if (isset($_POST['submit'])) {
    ...
    $checkQuery = "SELECT COUNT(*) as count FROM registerflat WHERE f_id = '$f_id'";
    $checkResult = $conn->query($checkQuery);
    $count = $checkResult->fetch_assoc()['count'];

    if ($count > 0) {
        echo '<script>alert("Sorry! Property already registered")</script>';
        echo '<script>window.location.href="flat.php";</script>';
        exit; // Stop further execution of the script
    }

    // Save the registration data in the registerp table
    $query = "INSERT INTO registerflat(name, phone, email, address, f_id, f_image) VALUES ('$name', '$phone', '$email', '$address', '$f_id', '$f_image')";
