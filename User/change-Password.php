...
$error = "";
if (isset($_GET['Reset'])) {
    if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM register WHERE CodeV='{$_GET['Reset']}'")) > 0) {
        if (isset($_POST['submit'])) {
            $Pass = mysqli_real_escape_string($conn, $_POST['Password']);
            $Confirme_Pass = mysqli_real_escape_string($conn, $_POST['Conf-Password']);
            if ($Pass === $Confirme_Pass) {
                $sql = "UPDATE register SET Password ='" . ($Pass) . "' WHERE CodeV='{$_GET['Reset']}'";
