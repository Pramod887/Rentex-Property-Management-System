...
if (isset($_GET['f_id'])) {
    ...
    if (!$itemExists) {
        ...
        $propertyDetails = getPropertyDetails($f_id);
        if ($propertyDetails !== null) {
            $cartItems[] = $propertyDetails;
            $_SESSION['cart2'] = $cartItems;
        }
    } else {
        // Item already exists in the cart
        $errorMsg = "Item already exists in the cart.";
        header("Location: cart2.php?error=" . urlencode($errorMsg));
        exit();
    }
...
function getPropertyDetails($f_id)
{
...
if (isset($_GET['delete'])) {
    ...
    if (isset($cartItems[$deleteIndex])) {
        ...
        $_SESSION['cart2'] = array_values($cartItems);
...
<?php if (count($cartItems) > 0): ?>
    ...
    <table>
        ...
        <?php foreach ($cartItems as $index => $item): ?>
            <tr>
                <td><img src="../seller/homeupload/<?php echo $item->f_image; ?>" alt="Property Image"></td>
                <td><?php echo $item->f_name; ?></td>
                <td><?php echo $item->f_phone; ?></td>
                <td><a href="cart2.php?delete=<?php echo $index; ?>" class="delete-button">Delete</a></td>
                <td><a href="property-singel1.php?f_id=<?php echo $item->f_id; ?>" class="delete-button">View</a></td>
            </tr>
        <?php endforeach; ?>
    </table>
<?php else: ?>
...
<!-- Back button -->
<a href="flat.php" class="back-button">Back</a>
